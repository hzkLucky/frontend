      ...
      <div class="total">
       ...
       <el-table :data="list">
          <el-table-column prop="time" label="时间" />
           <el-table-column prop="remark" label="收入明细" />
            <el-table-column prop="value" label="收入金额" />
       </el-table>
...
export default {
  components: {
   ...
   methods: {
     getData(){
       this.$axios.post('/gucp/IncomeList', {
       currentPage: this.$refs.pagination.currentPage ? this.$refs.pagination.currentPage : 1
     }).then( data => {
       this.sum = data.data.body.sum? data.data.body.sum : 0
